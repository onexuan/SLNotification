package com.sl.notification.toast;
...
import com.sl.notification.util.DisplayUtil;
...
/**
 * Created by wuhongqi on 17/2/8.
 */
public class SLToast implements IToast {
    private static Handler mHandler = new Handler();

    /**
     * 维护toast的队列
     */
    private static BlockingQueue<SLToast> mQueue = new LinkedBlockingQueue<SLToast>();
    ...
    public static IToast makeText(Context context, String text, long duration) {
        return new SLToast(context).setText(text).setDuration(duration)
    ...
    private static void activeQueue() {
        ...
        if (slToast == null) {
            ...
            mHandler.post(slToast.mShow);
            mHandler.postDelayed(slToast.mHide, slToast.mDurationMillis);
            mHandler.postDelayed(mActivite, slToast.mDurationMillis);
        }
    }
